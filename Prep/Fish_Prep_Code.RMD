...
sample1 <- read.csv(here::here("working","Sample_23JAN2024.csv"), quote = "")
dwr_sv <- read.csv(here::here("working","SiteVisit_23JAN2024.csv"), header=T, quote = "") # All site visits from the export page
str(dwr_sv)
...
dwr_sv_AFS= subset(dwr_sv, Visit.Date2 > "2022-10-01" & Visit.Date2 < "2023-06-30")
...
test_depth_handheld = filter(f1, f1$DepthSource != f1$DepthSource2) #the same samples as in depth_handheld should show up in this test. All did! -ED 23JAN2024
# It did! 
...
f1$DepthMeters <- ifelse((f1$Boat == "Mudsucker" |f1$Boat == "Chinook") & f1$GearTypeAbbreviation == "BIGLAM" & f1$DepthSource2 !="Handheld depth finder", f1$DepthOfWater2+0.3, f1$DepthOfWater2)
...
#Grabs any samples with the word "code" in the comments
# missing_code = f1[grepl('code', f1$Comments),]
# length(f1$GearTypeAbbreviation[which(f1$GearTypeAbbreviation=="OTTTRA")]) #60 OTTTRA
# 
# missing_code_otttra = filter(missing_code, missing_code$GearTypeAbbreviation == "OTTTRA")
# #Some were written as "code: ", this changes those into "code " for consistency
# missing_code_otttra$Comments2 <- gsub("code: ", "code ", missing_code_otttra$Comments)
# 
# 
# f1$SeineCode2 = f1$SeineCode
# 
# 
# for (i in 1:nrow(missing_code_otttra)){
#   if (grepl("code 1", missing_code_otttra$Comments2[i])){
#     missing_code_otttra$SeineCode[i]= "1. No twists; snags; or tears"
#   }
#   if (grepl("code 2", missing_code_otttra$Comments2[i])){
#     missing_code_otttra$SeineCode[i]= "2. Partial twist; snag; or tear"
#   }
#   if (grepl("code 3", missing_code_otttra$Comments2[i])){
#     missing_code_otttra$SeineCode[i]= "3. Complete twist; snag; or large tear"
#   }
#   if (grepl("code 4", missing_code_otttra$Comments2[i])){
#     missing_code_otttra$SeineCode[i]= "4. DTS"
#   }
# }
# 
# for (i in 1:nrow(f1)){
#   for (j in 1:nrow(missing_code_otttra)){
#     if (f1$SampleID_frp[i]==missing_code_otttra$SampleID_frp[j]){
#       f1$SeineCode2[i]= missing_code_otttra$SeineCode[j]
#     }
#   }
# }
...
#check that it worked
#test_seine_code2 = filter(f1, f1$SeineCode != f1$SeineCode2) # same number as missing_code_otttra

#test=f1[,c("VisitNo","SampleID_frp","GearTypeAbbreviation","SeineCode","SeineCode2")]
...
#path="C:/Users/edavidson/GitHub/AnnualReport_2022/AnnualBook_2022/imports_ED/Prep/working/FRP FISH Sample Inventory.xlsx"
#readxl::excel_sheets(path)

#Make sure to copy latest version into data prep folder (working directory)
fish_sample_inventory<- readxl::read_excel(here::here("working","2022_fish_sample_inventory.xlsx"))
fish_sample_inventory.2023<- read_excel(here::here("working","2023_fish_sample_inventory.xlsx"))


fish_sample_inventory <- fish_sample_inventory[,c("Location","Date","VisitNo","SampleID_frp","Gear Type","Seine Code")]
...
#Use to double check that nothing weird happened when converting the dates
test=fish_sample_inventory[,c("Visit.Date","Date")]


#Filter for desired year(s)/months

#Select year or date range desired
#fish_sample_inventory2= subset(fish_sample_inventory, Visit.Date > "2022-10-01" & Visit.Date < "2023-06-30")
fish_sample_inventory2<-fish_sample_inventory
names(fish_sample_inventory2)[which(colnames(fish_sample_inventory2)=="Seine Code")]<-"SeineCode"

#Remove Code 3/4
fish_sample_inventory3<-fish_sample_inventory2[!((fish_sample_inventory2$SeineCode == "4") | fish_sample_inventory2$SeineCode == "3") ,]
...
beachseine_all<-rbind(beachseine_crew, beachseine_ddb)
...
lamope$DepthMeters[is.na(lamope$DepthMeters)] <- 3.6
...
bind_check <- anti_join(fish1, f2, by=c('SampleID_frp', 'VisitNo', 'Comments')) #Should be zero
...
save(list=c("lamope_all","beachseine_all", "biglam","dwr_sv", "ss2","ss_fish", "sample1", "sitesample","f1", "f2", "fish_in_inverts","f_check", "fish1"),file=here::here("RData","fish_prep_code_10.RData")) #replace list=ls() with names of objects you want to keep, currently keeps all
...
other_gear=dplyr::filter(f2, f2$GearTypeAbbreviation == "OTTRA"|
             f2$GearTypeAbbreviation == "BOEFISH"|
             f2$GearTypeAbbreviation == "EFISH"|
             f2$GearTypeAbbreviation == "FYKE"|
             f2$GearTypeAbbreviation == "GILNET"|
...
castnet <- subset(f2, f2$GearTypeAbbreviation == "CAST") 
...
save(list=c("beachseine_all","lamope_all", "ottertrawl", "biglam","castnet", "f1", "f2", "sitesample", "stations", "fish1","fish_in_inverts","f_check2"),file=here::here("RData","fish_prep_code_11.RData")) #replace list=ls() with names of objects you want to keep, currently keeps all
...
catch1 <- read.csv(here::here("working","Fish_22JAN2024.csv"))
...
TotalCPUE5 <- TotalCPUE5[,c("SampleID_key","VisitNo", "SampleID_frp", "StartTime", "EndTime", "DateTime", "DepthMeters", "NetMeterStart", "NetMeterEnd", "TowDirection", "SeineCode2","SeineDeploymentType", "LengthOfTrawl", "Width", "GearTypeAbbreviation", "volume")]
