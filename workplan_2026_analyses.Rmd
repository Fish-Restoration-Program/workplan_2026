...
# Set output file path and size
png(filename = here::here("working/Figures", "rare_curve_plot_only.png"),
    width = 1200, height = 800)

# Set plot margins (adjust if legend was moved out)
par(mar = c(5, 4, 4, 2))  # default margins: bottom, left, top, right
...
plot(NULL,
     ...
     main = "Species Accumulation by Location × Gear × Season")
...
# Close the graphics device to write the file
dev.off()
#legend("right", legend = names(species_list), col = cols, lwd = 2, cex = 0.7)

# Open PNG device
png(filename = here::here("working/Figures", "legend_only.png"), width = 1000, height = 750)

# Create a new blank plot
plot.new()

# Add legend to fill the space
legend("center",
       legend = names(species_list),
       col = cols,
       lwd = 2,
       cex = 0.8,
       ncol = 3,          # 1 column (adjust to fit better)
...
ploX<-ggplot(accum_df, aes(x = Samples, y = Richness)) +
  geom_line(aes(color = Group), size = 1.2, show.legend = FALSE) +
...
ggsave(paste("RARE_CURVE2_LAM_COMBINED",".png",sep = ""),path = here::here("working/Figures"), ploX, width = 1600, height = 800, dpi = 100, units = "px")



# Get unique groups
group_list <- unique(accum_df$Group)

# Split groups into chunks
group_chunks <- split(group_list, ceiling(seq_along(group_list) / per_page))

# Loop over chunks and plot each
for (i in seq_along(group_chunks)) {
  subset_df <- accum_df %>% filter(Group %in% group_chunks[[i]])
  
  p <- ggplot(subset_df, aes(x = Samples, y = Richness)) +
    geom_line(aes(group = Group), color = "steelblue", size = 1) +
    facet_wrap(~ Group, ncol = n_cols) +
    labs(
      title = paste("Rarefaction Curves – Page", i, "of", length(group_chunks)),
      x = "Samples",
      y = "Species Richness"
    ) +
  ...
  ggsave(
    filename = paste0("rare_curve_manual_page_", i, ".png"),
    plot = p,
    path = here::here("working/Figures"),
...
```{r see which groups are already at the 80-90% mark (being sampled sufficiently)}
#Clear environment
rm(list = ls(all.names = TRUE)) #will clear all objects includes hidden objects.
...
if (any(predicted_richness >= target)) {
  ...
  if (all(y_vals < target)) {
    ...
    # Find smallest x where predicted richness ≥ target
...
# 🧠 Apply to all groups in your species_list (named list of sample-by-species matrices)
...
```{r determine how many samples are needed to reach 80% of the estimated species richness.}
#Clear environment
rm(list = ls(all.names = TRUE)) #will clear all objects includes hidden objects.
...
```{r Plot heatmap of sample numbers}
#Clear environment
rm(list = ls(all.names = TRUE)) #will clear all objects includes hidden objects.
...
```{r determine how many samples are needed to reach 90% of the estimated species richness.}
#Clear environment
rm(list = ls(all.names = TRUE)) #will clear all objects includes hidden objects.
...
```{r Plot heatmap of sample numbers 90%}
#Clear environment
rm(list = ls(all.names = TRUE)) #will clear all objects includes hidden objects.
